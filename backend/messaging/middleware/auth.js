...
/**
 * isOkHeaders middleware function that checks if headers are allowed
 * @param {Request} req HTTP request object
 * @param {Response} res HTTP response object
 * @param {NextFunction} next Function that should be called after this middleware
 */
async function isOkHeaders(req, res, next) {

  var okHeaders = ["POST", "GET", "PATCH", "DELETE"];
  if (!okHeaders.includes(req.method)) {
    res.set("Content-Type", "text/plain");
    res.status(405).send("Error: incorrect header.");
    return
  }
