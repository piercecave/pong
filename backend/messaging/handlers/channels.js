...
async function getAllChannels(req, res) {
  ...
  try {
    ...
    for (let i = 0; i < privateChannels.length; i++) {
      ...
      userChannels.push(privateChannelInfo);
    }
    ...
    for (let i = 0; i < publicChannels.length; i++) {
      ...
      if (publicChannel.ChannelName != "General") {
        ...
        const channelInfo = {
          ...
          "description": publicChannel.ChannelDescription,
          "private": publicChannel.PrivateChannel,
          "members": [],
          "createdAt": publicChannel.TimeCreated,
          "creator": publicChannelCreator.profile,
          "editedAt": publicChannel.LastUpdated
        }
...
async function sendMessage(req, res) {
  ...
  let newMessage;
  try {
    const qry = 'INSERT INTO Messages (ChannelID, Body, TimeCreated, Creator, LastUpdated) VALUES (?, ?, NOW(), ?, ?)';
    const qryResult = await db.query(qry, [channelID, req.body.body, user.id, null]);
...
async function updateChannel(req, res) {
  ...
  const rabbitPatchChannel = {
    "type": "channel-update",
    "channel": channel,
    "userIDs": memberIDs.members
  }

  const error = sendMessageToRabbitMQ(rabbitPatchChannel);
  if (error != null) {
    console.log("Error sending message to RabbitMQ: " + error);
    res.status(500).send("Error sending event message to RabbitMQ");
  }
...
async function removeUser(req, res) {
  ...
  res.status(201).type("text").send("User was removed from the specified channel.");
}
